...
このチュートリアルでは、**Windows版のCodexアプリをインストールし、研究室から配布されたOpenAI APIキーを使ってCodexを利用できるように設定します。**

Codexは、ChatGPT Plus/Pro/Business/Eduなどの有料ChatGPTアカウントでサインインして利用することもできます。しかし、研究室の学生全員が有料ChatGPTアカウントを持っているわけではないため、本チュートリアルでは、**研究室から学生一人一人に配布されているOpenAI APIキー**を使う方法で統一します。

### 1. Codexアプリをダウンロードする

OpenAIのCodex公式ページ（<https://chatgpt.com/codex/>）をWebブラウザで開きます。

画面に表示される **「Windows版をダウンロードする」**を選択して、Codexアプリをダウンロードしてください。

<img src="images/CodexHP.png" alt="Codex公式ページのWindows版ダウンロード画面" width="80%">

ダウンロードが完了したら、ダウンロードしたファイルを**ダブルクリック**してください。Codexアプリのインストールが開始されます。

### 2. サインイン方法を選択する

インストールが完了するとCodexが起動し、**「ChatGPTにサインイン」**画面が表示されます。

今回はChatGPTアカウントではなく、研究室で用意したOpenAI APIキーを使用するため、**「別の方法でサインイン」**を選択してください。

<img src="images/Sign-in.png" alt="Codexのサインイン画面で「別の方法でサインイン」を選択" width="60%">

### 3. OpenAI APIキーを設定する

「別の方法でサインイン」を選択すると、OpenAI APIキーの入力を求められます。

教員（研究室）から配布された**自分専用のOpenAI APIキー**を入力してください。
...
<img src="images/Codex.png" alt="Codexの初期画面" width="80%">

この画面が表示されたら、**CodexアプリのインストールとOpenAI APIの設定は完了です。**
...
│
├── Codexアプリ
│      └─ Javaプログラムの調査・修正・実行などに利用
│
└── java-llm-samples
       └─ JavaプログラムからLLMを呼び出すために利用
```
...
> **重要**
>
> **CodexアプリにAPIキーを設定しただけでは、`java-llm-samples`からOpenAI APIを利用するための設定は完了しません。**
...
-   **デフォルト権限：ON**
-   **フルアクセス：OFF**
-   **ファイルのデフォルトの開き先：VS Code**
-   **エージェントの環境：Windows ネイティブ**
...
> **重要**
>
> AIエージェントには、作業に必要な範囲だけアクセス権限を与えることを基本とします。
